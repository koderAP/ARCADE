...
def reset_level(level):
	...
	if path.exists(f'level{level}_data.csv'):
		df = pd.read_csv(f'level{level}_data.csv', header=None)
...
class Player():
	...
	def reset(self, x, y):
		...
		for num in range(1, 5):
			...
			img_right = pygame.transform.scale(img_right, (int(40 * screen_width/1000), int(80*screen_height/1000)))
		...
		for row in data:
			...
			for tile in row:
				...
				if tile == 5:
					...
					platform_group.add(platform)
				if tile == 6:
					lava = Lava(col_count * tile_size_x, row_count * tile_size_y + (tile_size_y // 2))
					lava_group.add(lava)
				if tile == 7:
...
class Platform(pygame.sprite.Sprite):
	def __init__(self, x, y, move_x, move_y):
		...
		self.image = pygame.transform.scale(img, (tile_size_x, tile_size_y // 2))
		self.rect = self.image.get_rect()
		self.rect.x = x
		self.rect.y = y
		self.move_counter = 0
...
class Coin(pygame.sprite.Sprite):
	def __init__(self, x, y):
		...
		self.image = pygame.transform.scale(img, (tile_size_x // 2, tile_size_y // 2))
		self.rect = self.image.get_rect()
